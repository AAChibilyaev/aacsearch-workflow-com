// src/widget.ts
var DEFAULT_HOST = "api.aacsearch.ru";
var GATEWAY_PATH = "/api/v1/scoped";
...
var CSS_ID = "aacsearch-theme";
var styleEl = null;
function injectTheme(cfg) {
  ...
  styleEl.textContent = `
        ...
        .aac-searchbox::before{content:'\u{1F50D}';position:absolute;left:.75rem;top:50%;transform:translateY(-50%);font-size:.9rem}
...
async function search(container, cfg) {
  ...
  const searchClient = { ...adapter.searchClient, search: adapter.searchClient.search };
  el.innerHTML = `
  ...
  widgetList.push(widgets.searchBox({
    container: getElement(root, ".aac-searchbox input"),
  ...
  if (cfg.autocomplete) {
    ...
    widgetList.push(widgets.hits({
      container: hitsContainer,
      templates: { item: cfg.renderHit || ((hit) => `<div class="aac-autocomplete-item aac-hit-title">${hit.title || hit.name || hit.id || ""}</div>`) }
    }));
    const searchInput = getElement(root, ".aac-searchbox input");
...
if (typeof window !== "undefined") {
  window.aacsearch = {
    search(container, cfg) {
      const widget = search(container, cfg);
      widget.then((w) => {
...
export { search2 as search };
//# sourceMappingURL=aacsearch-ui.mjs.map
//# sourceMappingURL=aacsearch-ui.mjs.map