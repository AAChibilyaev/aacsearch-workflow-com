...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`golden_queries\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`tenant_id\` integer,
  	\`name\` text NOT NULL,
  	\`collection\` text NOT NULL,
  	\`query\` text NOT NULL,
  	\`query_by\` text DEFAULT 'title',
  	\`expected_doc_ids\` text NOT NULL,
  	\`top_n\` numeric DEFAULT 5,
  	\`last_run_at\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`golden_queries_tenant_idx\` ON \`golden_queries\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`golden_queries_updated_at_idx\` ON \`golden_queries\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`golden_queries_created_at_idx\` ON \`golden_queries\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`golden_queries_id\` integer REFERENCES golden_queries(id);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_golden_queries_id_idx\` ON \`payload_locked_documents_rels\` (\`golden_queries_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`golden_queries\`;`)
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`reindex_jobs_id\`) REFERENCES \`reindex_jobs\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "products_id", "documents_id", "integrations_id", "invoices_id", "collection_definitions_id", "tenant_settings_id", "api_keys_id", "users_id", "media_id", "tenants_id", "reindex_jobs_id", "redirects_id", "search_id", "forms_id", "form_submissions_id", "notifications_id", "plugin_ai_instructions_id", "audit_log_id", "payload_mcp_api_keys_id") SELECT "id", "order", "parent_id", "path", "pages_id", "products_id", "documents_id", "integrations_id", "invoices_id", "collection_definitions_id", "tenant_settings_id", "api_keys_id", "users_id", "media_id", "tenants_id", "reindex_jobs_id", "redirects_id", "search_id", "forms_id", "form_submissions_id", "notifications_id", "plugin_ai_instructions_id", "audit_log_id", "payload_mcp_api_keys_id" FROM \`payload_locked_documents_rels\`;`)
