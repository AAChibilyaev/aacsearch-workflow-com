...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`integrations_tenant_idx\` ON \`integrations\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`integrations_integration_key_idx\` ON \`integrations\` (\`integration_key\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`integrations_connection_id_idx\` ON \`integrations\` (\`connection_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "products_id", "documents_id", "collection_definitions_id", "tenant_settings_id", "api_keys_id", "users_id", "media_id", "tenants_id", "redirects_id", "search_id", "forms_id", "form_submissions_id", "notifications_id", "plugin_ai_instructions_id", "payload_mcp_api_keys_id") SELECT "id", "order", "parent_id", "path", "pages_id", "products_id", "documents_id", "collection_definitions_id", "tenant_settings_id", "api_keys_id", "users_id", "media_id", "tenants_id", "redirects_id", "search_id", "forms_id", "form_submissions_id", "notifications_id", "plugin_ai_instructions_id", "payload_mcp_api_keys_id" FROM \`payload_locked_documents_rels\`;`)
