...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`documents\` (
  	...
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`tenant_id\`) REFERENCES \`tenants\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`definition_id\`) REFERENCES \`collection_definitions\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`documents_tenant_idx\` ON \`documents\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`documents_definition_idx\` ON \`documents\` (\`definition_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`documents_locales\` (
  	\`title\` text NOT NULL,
  	\`content\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`documents\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`documents_locales_locale_parent_id_unique\` ON \`documents_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`collection_definitions_fields_options\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`value\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`collection_definitions_fields\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`collection_definitions_fields_options_order_idx\` ON \`collection_definitions_fields_options\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`collection_definitions_fields_options_parent_id_idx\` ON \`collection_definitions_fields_options\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`collection_definitions_fields\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	...
  	\`localized\` integer DEFAULT false,
  	\`facet\` integer DEFAULT false,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`collection_definitions\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`collection_definitions_fields_order_idx\` ON \`collection_definitions_fields\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`collection_definitions_fields_parent_id_idx\` ON \`collection_definitions_fields\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`collection_definitions_fields_locales\` (
  	\`label\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`collection_definitions_fields_locales_locale_parent_id_uniqu\` ON \`collection_definitions_fields_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`collection_definitions\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`tenant_id\` integer,
  	\`slug\` text NOT NULL,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`collection_definitions_tenant_idx\` ON \`collection_definitions\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`collection_definitions_slug_idx\` ON \`collection_definitions\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`collection_definitions_updated_at_idx\` ON \`collection_definitions\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`collection_definitions_created_at_idx\` ON \`collection_definitions\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`collection_definitions_locales\` (
  	\`name\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`collection_definitions_locales_locale_parent_id_unique\` ON \`collection_definitions_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`tenant_settings_search_fields\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`field\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`tenant_settings\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`tenant_settings_search_fields_order_idx\` ON \`tenant_settings_search_fields\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`tenant_settings_search_fields_parent_id_idx\` ON \`tenant_settings_search_fields\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`tenant_settings_facet_fields\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`field\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`tenant_settings_facet_fields_order_idx\` ON \`tenant_settings_facet_fields\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`tenant_settings_facet_fields_parent_id_idx\` ON \`tenant_settings_facet_fields\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`tenant_settings_synonyms\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`root\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`tenant_settings_synonyms_order_idx\` ON \`tenant_settings_synonyms\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`tenant_settings_synonyms_parent_id_idx\` ON \`tenant_settings_synonyms\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`tenant_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`tenant_id\` integer,
  	\`brand_color\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`tenant_settings_tenant_idx\` ON \`tenant_settings\` (\`tenant_id\`);`)
  await db.run(sql`CREATE INDEX \`tenant_settings_updated_at_idx\` ON \`tenant_settings\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`tenant_settings_created_at_idx\` ON \`tenant_settings\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_mcp_api_keys\` (
  	...
  	\`documents_create\` integer DEFAULT false,
  	\`documents_update\` integer DEFAULT false,
  	\`documents_delete\` integer DEFAULT false,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`collection_definitions_id\` integer REFERENCES collection_definitions(id);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`tenant_settings_id\` integer REFERENCES tenant_settings(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`documents_locales\`;`)
  await db.run(sql`DROP TABLE \`collection_definitions_fields_options\`;`)
  await db.run(sql`DROP TABLE \`collection_definitions_fields\`;`)
  await db.run(sql`DROP TABLE \`collection_definitions_fields_locales\`;`)
  await db.run(sql`DROP TABLE \`collection_definitions\`;`)
  await db.run(sql`DROP TABLE \`collection_definitions_locales\`;`)
  await db.run(sql`DROP TABLE \`tenant_settings_search_fields\`;`)
  await db.run(sql`DROP TABLE \`tenant_settings_facet_fields\`;`)
  await db.run(sql`DROP TABLE \`tenant_settings_synonyms\`;`)
  await db.run(sql`DROP TABLE \`tenant_settings\`;`)
