...
import type { TenantOption } from './RelevancePanel'

import { RelevancePanel } from './RelevancePanel'
import { t } from './shared'

/**
 * Custom admin view at /admin/relevance. Server component: resolves the
 * acting tenant (multi-tenant plugin cookie -> user membership -> first
 * option) and renders the client RelevancePanel — a visual builder over the
 * existing `tenant-settings` synonyms/curation/stopwords array fields. This
 * is purely a nicer front-end over that existing data; the afterChange sync
 * to the search engine already happens elsewhere and is untouched here.
 */
export async function RelevanceView({ initPageResult, params, searchParams }: AdminViewServerProps) {
  ...
  return (
    ...
    >
      <Gutter>
        ...
        <RelevancePanel initialTenantId={initialTenantId} lang={lang} tenantOptions={tenantOptions} />
      </Gutter>
    </DefaultTemplate>
  )
}
