...
import type { TenantOption } from './SearchPanel'

import { t } from './i18n'
import { SearchPanel } from './SearchPanel'

/**
 * Custom admin view at /admin/search. Server component: resolves the acting
 * tenant (multi-tenant plugin cookie -> user membership -> first option) and
 * renders the client SearchPanel, which talks to the white-label
 * /api/search/* + /api/v1/search endpoints. The search engine is an
 * implementation detail — nothing here names it.
 */
export async function SearchView({ initPageResult, params, searchParams }: AdminViewServerProps) {
  ...
  return (
    ...
    >
      <Gutter>
        ...
        <SearchPanel initialTenantId={initialTenantId} lang={lang} tenantOptions={tenantOptions} />
      </Gutter>
    </DefaultTemplate>
  )
}
